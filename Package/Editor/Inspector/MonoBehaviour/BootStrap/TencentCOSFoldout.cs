...
namespace ZFramework.Editor
{
    public class TencentCOSFoldout : FadeFoldout
    {
        ...
        protected override void OnGUI()
        {
            ...
            {
                COS.PUTObject("temp.txt", Encoding.UTF8.GetBytes("测试上传一个txt文件"));
            }
            if (GUILayout.Button("判断指定对象是否存在和有权限"))
            {
                COS.HEADObject("temp.txt");
            }
            if (GUILayout.Button("将 COS 存储桶中的对象（Object）下载至本地"))
            {
                COS.GETObject("temp.txt");
            }
            if (GUILayout.Button("分块下载"))
            {
                COS.GETObjectRange("temp.txt", "0", "14");
            }
            if (GUILayout.Button("删除一个指定的对象（Object）"))
            {
                COS.DELETEObject("temp.txt");
            }
            ...
            {
                COS.MultipartUpload_Initiate("test.ab");
            }
            if (GUILayout.Button("分块上传到 COS"))
            {
                COS.UploadPart("test.ab", uploadID, 1, Encoding.UTF8.GetBytes("测试分块上传,就传一块试一下"));
            }
            ...
            {
                COS.AbortMultipartUpload(abName, uploadID);
            }
            if (GUILayout.Button("查询特定分块上传中的已上传的块"))
            {
                COS.ListParts("test.ab", uploadID);
            }
        }
