...
namespace ZFramework
{
    ...
    public class CosFileServer : IFileServer
    {
        ...
        {
            var APPID = "1300651736";
            var SecretId = "AKIDDZptthdNvMceR45wZ2JRYhrxudDalkmu";
            var SecretKey = "YEqRq7jmfMwRRGE6wlYM6cfipw352teK";
            var Region = "ap-guangzhou";
            var Bucket = "daschow";
            cosServer = new TencentCOS(APPID,SecretId,SecretKey,Region,Bucket);
        }
        ...
        public async ATask DownloadFileRange(string filePath, Stream stream)
        {
            ...
            {
                int size = a.Item2;
                Log.Info("准备流式下载 -->" + size);
                int index = 0;
                int bufferLenght = 1024 * 1024 * 4;//每块1M下载 4194304
                while (true)
                {
                    if (index + bufferLenght < size)
                    {
                        byte[] bytes = await cosServer.GETObjectRange(filePath, index.ToString(), (index + (bufferLenght - 1)).ToString());
                    ...
                    else//最后一块
                    {
                        ...
                        index += bytes.Length;
                        Log.Info("download succses -->" + index);
                        return;
                    }
                }
            }
