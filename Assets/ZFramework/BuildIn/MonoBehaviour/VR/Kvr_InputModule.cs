...
namespace KVR_UI
{
    public class Kvr_InputModule: PointerInputModule
    {
        public static List<Kvr_UIPointer> pointers = new List<Kvr_UIPointer>();
        ...
        public static void AddPoint(Kvr_UIPointer point)
        {
            if (!pointers.Contains(point))
                pointers.Add(point);
        }

        public static void RemovePoint(Kvr_UIPointer point)
        {
            if (pointers.Contains(point))
        ...
        public override void Process()
        {
            ...
            {
                Kvr_UIPointer pointer = pointers[i];
        ...
        protected virtual List<RaycastResult> CheckRaycasts(Kvr_UIPointer pointer)
        {
        ...
        protected virtual bool NoValidCollision(Kvr_UIPointer pointer, List<RaycastResult> results)
        {
            return (results.Count == 0 || !CheckTransformTree(results[0].gameObject.transform, pointer.pointerEventData.pointerEnter.transform));
        }


        protected virtual bool IsHovering(Kvr_UIPointer pointer)
        {
        ...
        {
            var canvasCheck = obj.GetComponentInParent<Kvr_UICanvas>();
            return (canvasCheck && canvasCheck.enabled ? true : false);
        }

        protected virtual void CheckPointerHoverClick(Kvr_UIPointer pointer, List<RaycastResult> results)
        {
        ...
        protected virtual void Hover(Kvr_UIPointer pointer, List<RaycastResult> results)
        {
        ...
        protected virtual void Click(Kvr_UIPointer pointer, List<RaycastResult> results)
        {
            switch (pointer.clickMethod)
            {
                case Kvr_UIPointer.ClickMethods.ClickOnButtonUp:
                    ClickOnUp(pointer, results);
                    break;
                case Kvr_UIPointer.ClickMethods.ClickOnButtonDown:
                    ClickOnDown(pointer, results);
                    break;
            }
        }
        protected virtual void ClickOnUp(Kvr_UIPointer pointer, List<RaycastResult> results)
        {
        ...
        protected virtual void ClickOnDown(Kvr_UIPointer pointer, List<RaycastResult> results, bool forceClick = false)
        {
        ...
        protected virtual bool IsEligibleClick(Kvr_UIPointer pointer, List<RaycastResult> results)
        {
        ...
        protected virtual bool AttemptClick(Kvr_UIPointer pointer)
        {
        ...
        protected virtual void Drag(Kvr_UIPointer pointer, List<RaycastResult> results)
        {
